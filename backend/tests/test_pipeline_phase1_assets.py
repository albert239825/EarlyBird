...
def test_generate_research_and_script_assets_writes_files(tmp_podcast_dir: Path, state, openai_api_key, perplexity_api_key, monkeypatch):
    ...
    # Scraper returns a headline wrapped in tags (existing parsing logic)
    def _fake_get_top_headlines(category):
        return {"content": "<HEADLINE>Test Headline</HEADLINE>", "citations": []}

    monkeypatch.setattr(pipeline.scraper, "get_top_headlines", _fake_get_top_headlines)

    # Researcher returns the expected nested structure
    ...
    # HQ script writer returns deterministic utterances
    ...
    monkeypatch.setattr(pipeline.openai_script_writer, "write_story_script", _fake_write_story_script)
    ...
    assert out["stories"][0]["title"] == "Test Headline"
    ...
    script = json.loads((tmp_podcast_dir / "script" / "story_0.json").read_text(encoding="utf-8"))
    assert script["story_index"] == 0
    assert len(script["utterances"]) == 2

    # State updated
    assert len(state.articles) == 1
    assert state.articles[0]["article_data"].title == "Test Headline"
...
def test_generate_research_and_script_assets_integration_full_pipeline(tmp_podcast_dir: Path, state):
    """
    ...
    podcast_meta = json.loads((tmp_podcast_dir / "podcast.json").read_text(encoding="utf-8"))
    headline = podcast_meta["stories"][0]["title"]
    print(f"{'='*60}")
    print(f"HEADLINE (from Perplexity):")
    print(f"  {headline}")
    print(f"{'='*60}\n")

    # Read and display research doc
    research_md = (tmp_podcast_dir / "research" / "story_0.md").read_text(encoding="utf-8")
    print(f"{'='*60}")
    print(f"RESEARCH DOCUMENT (from Perplexity):")
    print(f"{'='*60}")
    ...
    print(f"{'='*60}")
    for i, u in enumerate(utterances[:8]):  # Show first 8
